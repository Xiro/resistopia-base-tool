...
class <?= $controllerClass ?> extends <?= StringHelper::basename($generator->baseControllerClass) . "\n" ?>
{
    ...
    public function actionIndex()
    {
        ...
        $confirmationProvider = $searchModel->search([]);
        $confirmationProvider->query->where(["approved" => null]);
...
<?php endif; ?>
    ...
    /**
     * Show all items that need approval
     * @return string
     */
    public function actionApproveIndex()
    {
        $searchModel = new <?= isset($searchModelAlias) ? $searchModelAlias : $searchModelClass ?>();
        $dataProvider = $searchModel->search(Yii::$app->request->queryParams);
        $dataProvider->query->where(["approved" => null]);
        $dataProvider->setPagination(false);

        return $this->render("approve-index", [
            "dataProvider" => $dataProvider
        ]);
    }

    /**
     * Approve or decline an item
     * @param int $id
     * @return string
     * @throws NotFoundHttpException if the model cannot be found
     */
...
<?php endif; ?>
        ...
        if ($model->load($post) && $model->save()) {
            if ($model->approved == 0) {
                return JsonResponse::success("Entry was declined");
            }
            return JsonResponse::success("Entry was approved");
        }
